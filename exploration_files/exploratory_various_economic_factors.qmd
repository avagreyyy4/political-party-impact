...
```{r}
various_factors <- read_csv("/Users/avagrey/political-party-impact/data/raw/state_factors.csv")
...
library(redav)
various_factors2 |>
  filter(State == 'Alabama')|> plot_missing(percent = FALSE)
```
...
ggplot(various_factors_NA, aes(x = key, y = fct_rev(year), fill = missing)) +
...
data_ready <- data_ready |>
  mutate(outlier_label = ifelse(
    abs(dif_PC_PCE_US) > 4000 | abs(dif_PC_DPI_US) > 4000,
    State,
    NA
  ))
...
data_ready <- data_ready |>
  ...
  mutate(
    outlier_label = ifelse(
      rank(-abs(dif_PC_PCE_US)) <= 2 | rank(-abs(dif_PC_DPI_US)) <= 2,
      State,
      NA
    )
  )
